...
const KEY_PRICE = 50; // coins per key

module.exports = {
  data: new SlashCommandBuilder()
    .setName('buy')
    .setDescription('Buy crates or keys with coins')
    ...
    .addSubcommand(subcommand =>
      subcommand
        .setName('key')
        .setDescription(`Buy keys with coins (${KEY_PRICE} coins per key)`)
        .addIntegerOption(option =>
          option.setName('amount')
            .setDescription('Number of keys to buy')
            .setRequired(false)
            .setMinValue(1)
            .setMaxValue(50))),
  ...
  async execute(interaction) {
    ...
    if (subcommand === 'crate') {
      ...
      const amount = interaction.options.getInteger('amount') || 1;
      const totalCost = KEY_PRICE * amount;
      ...
      await interaction.reply(`✅ Purchased ${amount}x 🔑 Key${amount > 1 ? 's' : ''} for ${totalCost} coins!\n**New Balance:** ${user.coins} coins | ${user.keys} keys`);
    }
  }
};